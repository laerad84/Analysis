...
main(int argc,char** argv)
{
  ...
  outputFilename      = Form("%s/WavCalibration_Data/CalibrationADV_%04d_%d_%s.root",
			     ROOTFILE_WAV.c_str(),RunNumber,IterationNumber,CalibrationName.c_str());
  
  calibrationFilename = Form("%s/WavCalibration_Data/CalibrationFactorADV_%d_%s.dat",
			     ROOTFILE_WAV.c_str(),IterationNumber,CalibrationName.c_str());
  
  std::cout<<"CalibrationName   : "<< CalibrationName      << std::endl;
  ...
  TFile*   tfTempCorr  =new TFile(Form("%s/Data/Temperature_Factor/TemperatureCorrectionFactor.root",
				       ANALYSISLIB.c_str()));
