...
int main( int argc, char** argv){
  ...
  Int_t   nTrack;
  Double_t v[300][3];
  Double_t end_v[300][3];
  Double_t p[300][3];
  Double_t end_p[300][3];
  ...
  tr->SetBranchAddress("end_v",end_v);//nTrack
  tr->SetBranchAddress("p",p);//nTrack
  tr->SetBranchAddress("end_p",end_p);//nTrack
  ...
  for( int ievent  =0; ievent < tr->GetEntries(); ievent++){
    ...
    double zCol = -21000+7000;
    double xCol = zCol*p[0][0]/p[0][2]+v[0][0];
    double yCol = zCol*p[0][1]/p[0][2]+v[0][1];
    //exclude core 
    if( abs( xCol ) < 15.2/2. ||
	abs( yCol ) < 25.0/2. ){
      continue; 
