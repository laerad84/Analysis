...
int main( int argc, char** argv){
  ...
  for( int ievent  =0; ievent < nTotal; ievent++){
    ...
    for( int i = 0; i< plist.size(); i++,pit++){
      if( TMath::Abs((*pit).g1().x()) < 150 &&
	  TMath::Abs((*pit).g1().y()) < 150 ){
	CutCondition |= 1 << cgammaPosIn;
      }
      if( TMath::Abs((*pit).g2().x()) < 150 &&
	  TMath::Abs((*pit).g2().y()) < 150 ){
	CutCondition |= 1 << cgammaPosIn;
      }
      if( TMath::Sqrt((*pit).g1().x()*(*pit).g1().x()+(*pit).g1().y()*(*pit).g1().y()) > 850){
	CutCondition |= 1 << cgammaPosOut;
      }
      if( TMath::Sqrt((*pit).g2().x()*(*pit).g2().x()+(*pit).g2().y()*(*pit).g2().y()) > 850){
	CutCondition |= 1 << cgammaPosOut;
      }      
    }
    ...
    for( int i  =0; i< plist.size()*2; i++,git++){
      ECenter = 0;
      EGamma = (*git).edep();
