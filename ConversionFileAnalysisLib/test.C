...
void test(){
  ...
  const double SumOffset[10]={29.4336,-10.1215,-8.90709,0,-5.25396,-27.1452,-39.6879,19.5912,3.2765,20.1921};
  ...
  TH1D* hisSumDelta[10][10];
  {
    ...
    for( int i = 0; i< 10; i++ ){
      for( int j = 0; j< 10; j++){
	hisSumDelta[i][j] = new TH1D(Form("hisSumDelta_%d_%d",i,j),Form("Sum of Channel %d and %d", i, j), 200, -100, 100);
      }
    }    
  }
  ...
  for( int i = 0; i< 20000; i++){
    ...
    for( int iIndex  = 0 ; iIndex < 10; iIndex++){
      if( cosmicOut[iIndex] < 1000 || cosmicOut[iIndex+10] < 1000){ continue; }
      for( int jIndex = iIndex+1 ; jIndex < 10; jIndex++ ){
	if( cosmicOut[jIndex] < 1000 || cosmicOut[jIndex+10] < 1000 ){ continue; }
	hisSumDelta[iIndex][jIndex]->Fill ( cosmicTime[iIndex] + cosmicTime[iIndex+10] - cosmicTime[jIndex] -cosmicTime[jIndex+10] - SumOffset[iIndex] +SumOffset[jIndex]);
      }
    }
  ...
  std::ofstream ofs("HistDelta_afteradj.txt");
  std::ofstream ofs1("hisSumDelta_afteradj.txt");

  TPostScript* ps = new TPostScript("test_afteradj.ps",111);
  ...
  for( int i = 0; i< 10; i++){
    ...
    for( int j = 0; j< 10; j++){
      can->cd( j+1 );
      hisSumDelta[i][j]->Draw();
      ofs1 << i << "\t"
	   << j << "\t" 
	   << hisSumDelta[i][j]->GetEntries()   << "\t"
	   << hisSumDelta[i][j]->GetMean()      << "\t"
	   << hisSumDelta[i][j]->GetMeanError() << std::endl;
    }
    can->Update();
    can->Modified();
  }	       
