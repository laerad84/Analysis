...
void fcn(Int_t &npar, Double_t *gin, Double_t &f, Double_t *par, Int_t iflag)
{
  ...
  if( cycle%100 == 0 ){
    std::cerr << "Iteration:" << cycle << "\tchisquare: " << f << "\t";
    for( int index=0; index<N; index++ ){
      if( index%500==0 ){
	std::cerr << par[index] << " ";
      }
...
void cosmicAnalysis3( const char* kind, const int region )
{
  ...
  for( int index=0; index<N; index++ ){
    if( idFlag[index] ){
      if( region ){
	if( gMinuit->DefineParameter( index, Form("p%d",index), offsets[index],
				      0.1, minValue, maxValue ) ){
	  std::cerr<< "DEBUG : index=" << index <<std::endl;
	  return;
	}
      }else{
	if( index < 2240 ){
 ...
 /*
   ...
   crystal[index] = index;
   output << crystal[index] << "\t" << offset[index] << "\t" << error[index] <<std::endl;
   }
...
int main(){
  ...
  /*
    std::ifstream ifs("hisSumDelta.txt");
    double mean;
    double error; 
    double entries;
    int iID;
  ...
  */
  ...
  for( int index=0; index<N; index++ ){
    ...
    if( idFlag[index] ){
      if( index < 2240 ){
	  offsets[index] -= 14./(2240.+238.)*238.;
      }else{
	offsets[index] += 14./(2240.+238.)*2240.;
      }
  ...
  for ( int index = 0; index < N ;index ++){
    if( idFlag[index] ){
      if( index == 1120 ){
	gMinuit->DefineParameter( index, Form("p%d",index), 0, 1., -2., 2. );
	gMinuit->FixParameter( index );
      }else if( index < 2240){
	gMinuit->DefineParameter( index ,Form("p%d", index ), offsets[index], 0.1, minValue, maxValue );
      }else{
	gMinuit->DefineParameter( index ,Form("p%d", index ), offsets[index], 0.1, minValue+14, maxValue+14 );
      }
    }else{
      gMinuit->DefineParameter( index, Form("p%d",index), 0, 1., -2., 2. );
      gMinuit->FixParameter( index );
