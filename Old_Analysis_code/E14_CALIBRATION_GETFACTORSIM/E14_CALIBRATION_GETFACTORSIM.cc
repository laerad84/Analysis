...
int main( int argc ,char** argv){
  ...
  */
  ...
  if( CalibrationNumber!=0 ){
    std::ifstream ifs(Form("SimCalibration_Data/CalibrationFactor_%d.dat",			   
			   CalibrationNumber));
  ...
  std::ofstream ofs(Form("SimCalibration_Data/CalibrationFactor_%d.dat",
			 nextCalNum));
  std::ofstream ofs1(Form("SimCalibration_Data/CalibrationStatics_%d.dat",
			  nextCalNum));
  //std::string listFilename=runListFile.substr(runListFile.find_last_of('/')+1);
  //std::cout<< listFilename << std::endl;
  TFile* tfOut 
    = new TFile(Form("SimCalibration_Data/CalhistList_%d.root",
		     CalibrationNumber),
		"RECREATE");
