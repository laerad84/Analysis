...
main( int argc ,char ** argv ){
  ...
  TGraphErrors* grDeltaADJ        = new TGraphErrors(); 
  ...
  */
  ...
  Double_t TimeOffsetCosmicMean[2716]={0};
  ...
  std::ifstream ifsCosmic("CosmicOut_V4_TimeDeltaResolution.dat");
  while( ifsCosmic >> tID >> tOffset >> tOffsetSigma ){
    if( abs(tOffset ) > 30 ){ continue; }
    TimeOffsetCosmicMean[tID] = tOffset;
  ...
  for( int ievent  = 0; ievent < entries ; ievent++){
	...
	if( TimeOffsetSigma > 0  && Converter->ConvertToEnergy( reader->CsiID[ich] ,reader->CsiSignal[ich] ) > 3 ){
	  ...
	  grDeltaADJ       ->SetPoint( grDeltaADJ->GetN()     , y, reader->CsiHHTime[ ich ] - TimeOffset[ reader->CsiID[ ich ] ] + TimeOffsetCrystalPosition[ reader->CsiID[ ich ] ] - TimeOffsetCosmicMean[ reader->CsiID[ ich ] ]);
	  grDeltaADJ       ->SetPointError( grDeltaADJ->GetN()-1, 12.5, TimeOffsetCosmicSigma[ reader->CsiID[ich] ] );
	}
	nCSIDigi++;
      }
    }
