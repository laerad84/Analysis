...
main( int argc ,char ** argv ){
  ...
  */
  ...
  std::ifstream ifst("CosmicOut_V2_TimeDeltaResolution.dat");
  double Delta[2716]      = {0};
  ...
  */
  ...
  TFile* tfout = new TFile("CosmicOut_V3.root","RECREATE");
  ...
  */
  ...
  for( int ievent  = 0; ievent < entries ; ievent++){
	...
	if( TimeOffsetSigma > 0  && Converter->ConvertToEnergy( reader->CsiID[ich] ,reader->CsiSignal[ich] ) > 7 ){
    ...
    if( grHeightTimeNoADJ->GetN() <5 ){ continue; }
