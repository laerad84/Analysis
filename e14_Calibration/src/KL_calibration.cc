...
int CalEnergy_idv(Klong &KL_prefit,TH1D** his_Fit, TH1D** his_CSI){
  ...
  for( unsigned int idx = 0; idx < KL_prefit.pi0().size() ; idx++ ){
    ...
    double corrfactor = clustnocorr/KL_prefit.pi0()[idx].g1().e();
    //corr   = (kl.pi0()[kl.pi0().size() - 1].g2().e())/KL_prefit.pi0()[idx].g1().e()/corrfactor;
    //err    = (kl.pi0()[kl.pi0().size() - 1].g2().sigmaE())/KL_prefit.pi0()[idx].g1().e()/corrfactor;     
   corr = (kl.pi0()[kl.pi0().size() - 1].g2().e()) / (KL_prefit.pi0()[idx].g1().e() );
    ...
    for( i_id = KL_prefit.pi0()[idx].g1().clusterIdVec().begin(),
	 ...
	 i_id != KL_prefit.pi0()[idx].g1().clusterIdVec().end(); i_id++,i_e++ ) {
      ...
      if( *i_id != *KL_prefit.pi0()[idx].g1().clusterIdVec().begin() && *i_e >=KL_prefit.pi0()[idx].g1().e()*0.2 ){
	flag = 1;
      }
    }
	...
	for( jj_id = KL_prefit.pi0()[idx].g1().clusterIdVec().begin(),
	  ...
	  if((!( *i_id != *KL_prefit.pi0()[idx].g1().clusterIdVec().begin() && csi_energy[*i_id] >= KL_prefit.pi0()[idx].g1().e()*0.2))){
	    clustE += *jj_e;
	  }
	}
    ...
    corrfactor = clustnocorr/KL_prefit.pi0()[idx].g1().e();
    //corr   = (kl.pi0()[kl.pi0().size() -1].g2().e())/KL_prefit.pi0()[idx].g1().e()/corrfactor;
    //err  = (kl.pi0()[kl.pi0().size() - 1].g2().sigmaE()) / KL_prefit.pi0()[idx].g1().e()/corrfactor; 
    ...
    for( i_id = KL_prefit.pi0()[idx].g2().clusterIdVec().begin(),i_e = KL_prefit.pi0()[idx].g2().clusterEVec().begin();
	 ...
	 i_id++,i_e++ ) {
      ...
      if( *i_id != *KL_prefit.pi0()[idx].g2().clusterIdVec().begin() && *i_e >=KL_prefit.pi0()[idx].g2().e()*0.2 ){
	flag = 1;
      }
    }
