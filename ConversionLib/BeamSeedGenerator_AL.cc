...
int main( int argc, char** argv ){
  ...
  TFile* tf = new TFile("beam01540.root");
  ...
  TFile* tfout = new TFile("beam01540_Reduced.root","recreate");
  ...
  for( int i = 0; i< tr->GetEntries(); i++ ){
	...
	totene += PHEP0[itrack];
      ...
      if( totene < 0.1 ){ continue; }
      for( int itrack = 0; itrack < NHEP; itrack++){
	AISTHEP[itrack] = ISTHEP[itrack];
	AIDHEP[itrack]  = IDHEP[itrack];
	AJDAHEP1[itrack] = JDAHEP1[itrack];
	AJDAHEP2[itrack] = JDAHEP2[itrack];
	APHEP1[ANHEP] = PHEP1[itrack];
	APHEP2[ANHEP] = PHEP2[itrack];
	APHEP3[ANHEP] = PHEP3[itrack];
	AVHEP1[ANHEP] = VHEP1[itrack];
	AVHEP2[ANHEP] = VHEP2[itrack];
	AVHEP3[ANHEP] = VHEP3[itrack];
	AVHEP4[ANHEP] = VHEP4[itrack];
	
	APHEP4[ANHEP] = PHEP4[itrack];
	APHEP5[ANHEP] = PHEP5[itrack];
	APHEP0[ANHEP] = PHEP0[itrack];
	ANHEP++;
	/*
