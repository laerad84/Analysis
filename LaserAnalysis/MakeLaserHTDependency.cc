...
int main( int argc, char** argv ){
  ...
  ch->Add(Form("%s/run_wav_%d.root",ROOTFILE_WAV.c_str(),4747));
  ...
  TFile* tfOut = new TFile("LaserHT.root","RECREATE");
  ...
  for( int ievt = 0; ievt < nEntries; ievt++){
    ...
    for( int iCsi = 0; iCsi < CsiNumber; iCsi++){
      hisLaserHT[iCsi]->Fill(CsiSignal[iCsi],CsiTime[iCsi] - LaserTime[0] );
      if( CsiSignal[iCsi] <400 && CsiSignal[iCsi] > 200 ){ 
	hisLaserT0[iCsi]->Fill( CsiTime[iCsi]-LaserTime[0] );
      }
    }
  }
  for( int i = 0; i< 2716; i++){
