...
#include "Pi0_RUN_TRIGGERSIM/user_func.h"
...
main(int argc,char** argv){
  ...
  for( int ievt=0; ievt<nloop; ievt++ ){
    ...
    if( ReadSum->SciEne[0] < 1 ){continue;}
    double CVtotE=0;
    for( int icv = 0; icv < ReadSum->CVNumber; ++icv){
      CVtotE += ReadSum->CVEne[icv];
    }
    if( CVtotE >0.8 ){continue;}
