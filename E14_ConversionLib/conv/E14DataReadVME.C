...
#if !defined(__CINT__)
ClassImp(E14DataReadVME)
#endif

  using namespace std;

E14DataReadVME::E14DataReadVME()
{
  EventNo = -1;
}
...
void E14DataReadVME::initializeDataValues(){
...
void E14DataReadVME::FileOpen( char *InFileName )
{
...
int E14DataReadVME::HeaderRead(){
  ...
  if( dp==0x12341234 ){
    ...
    fread((void *)&CrateID,sizeof(int),1,fout);
...
int E14DataReadVME::DataRead( int sn ){
  ...
  for(int n=0;n<nFADC;n++){	
	...
	std::cerr<<"not header data in loop:"<<BufferLoopNo<<", event"<<sn<<"FADC:"<<n<<"data="<<headerdata<<std::endl;
  ...
  for(int n=0;n<nFADC;n++){	
	...
	std::cerr<<"not header data in loop:"<<l<<", event"<<m<<"FADC:"<<n<<"data="headerdata<<std::endl;
	b_error_header[n]=kTRUE;
	continue;
	}*/
  ...
  for(int n=0;n<nFADC;n++){
    ...
    TimeStamp[n] = ((headersample[n][1]&0x1000)<<16)+(headersample[n][2]<<14)+headersample[n][3];
...
void E14DataReadVME::TimeStampJudge(){
...
int  E14DataReadVME::FileOpenConv( char *InFileName )
{
...
void E14DataReadVME::DataReadConv( int ievt )
{
