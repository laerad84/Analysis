...
bool PeakCompensater::Init( int version ){
  ...
  if( version == 0){ 
    ...
    m_gr[0] = (TGraph*)tf->Get("heightLinearityLaser_0");//for most of Small 
    m_gr[1] = (TGraph*)tf->Get("heightLinearityLaser_1");//for some of Small
    m_gr[2] = (TGraph*)tf->Get("heightLinearityLaser_2");//for All  of Large
    for( int i = 0; i< 3; i++){
      m_spl[i] = new TSpline3(Form("Linearity%d",i),m_gr[i]);      
      for( int j = 0; j< 160; j++){
	double x = j*100;
