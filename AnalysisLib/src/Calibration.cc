...
int Calibration::CalEnergy_idv(std::vector<Klong> &KL_VecInit){
  ...
  if( rst != 0 ){ 
    //std::cout<< "KL_Init" << std::endl;
    return m_nCalibrated;
  }
  rst = SelectKL_prefit();
  if( rst != 0 ){
    //std::cout<< "KL_prefit"<< std::endl;
    return m_nCalibrated;
  }
...
int Calibration::SelectKL_prefit(){
  ...
  if( KL_prefit.chisqZ() > 10 ){ 
    m_FlagKL_prefit |= 1;
  } 
  if( fabs( KL_prefit.m() - MASS_KL ) > klmassdiff ){
    m_FlagKL_prefit |=2;
  }
  if( KL_prefit.pi0().size() != 3){ m_FlagKL_prefit |=4;}
  ...
  for( unsigned int idx  = 0; idx < KL_prefit.pi0().size(); ++idx){
    if( fabs( KL_prefit.pi0()[idx].m() -MASS_PI0 ) > pimassdiff){ m_FlagKL_prefit |= 16;}
    pos[idx*2]   = KL_prefit.pi0()[idx].g1().pos();
    pos[idx*2+1] = KL_prefit.pi0()[idx].g2().pos();    
  }
  for( int i = 0; i < 6; ++i ){
    for( int j = i+1; j < 6; ++j){
      if(( pos[i] - pos[j] ).perp() < 140.){
	m_FlagKL_prefit |= 32;
      }
    }
  }  
  if( KL_Vec.size() > 1 ){
    if( KL_Vec[1].chisqZ() - KL_Vec[0].chisqZ() < 5){
      m_FlagKL_prefit |= 64; 
    }
  }
...
int Calibration::SelectKL(int idx, int GammaIndex){
  int Flag = 0;
  if( m_chisq[idx*2 + GammaIndex]/2. > 5. ){ Flag |= 1; }
  if( fabs( KL.m() - MASS_KL) >  klmassdiff ){ Flag |= 2; }
  if( fabs( KL.pi0()[KL.pi0().size() -1 ].m() - MASS_PI0 ) > pimassdiff ){ Flag |= 4; }
  if( fabs( KL.vz() - KL_prefit.vz() ) > 200. ){ Flag |= 8;}
