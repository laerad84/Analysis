...
int Calibration::InitChisqGraph(){
  for( int i =0; i< 6; i++){
    grChisq[i] = new TGraph();
    grChisq[i]->SetNameTitle(Form("ChisqGraph_%d",i),Form("ChisqGraph_%d;Energy[MeV];Chisq",i));
  }
  return 0;
}
...
double Calibration::Calibrate(){
  ...
  for(it=0;it<MAX_ITERACTIONS;it++) {		// Global minizing loop
    ...
    if( error_flag == 0 && chisq >=0. ){
      grChisq[grIndex]->SetPoint(grChisq[grIndex]->GetN(),v(4),chisq);
    }
